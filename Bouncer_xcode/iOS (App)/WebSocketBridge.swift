...
// Persistent per-install user id, stored in the Keychain so it survives app
// reinstalls. Generated lazily on first read.
enum BouncerUserId {
    private static let service = "com.bouncer.userid"
    private static let account = "default"

    static func get() -> String {
        if let existing = read() {
            print("[BouncerUserId] using existing user_id=\(existing)")
            return existing
        }
        let new = UUID().uuidString
        write(new)
        print("[BouncerUserId] generated new user_id=\(new)")
        return new
    }
...
class WebSocketBridge: NSObject, URLSessionWebSocketDelegate {
    ...
    private static func appendingUserId(to url: URL) -> URL? {
        guard var comps = URLComponents(url: url, resolvingAgainstBaseURL: false) else { return url }
        var items = comps.queryItems ?? []
        if items.contains(where: { $0.name == "user_id" }) { return url }
        items.append(URLQueryItem(name: "user_id", value: BouncerUserId.get()))
