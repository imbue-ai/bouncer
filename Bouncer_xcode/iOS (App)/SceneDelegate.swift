...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    /// `bouncer://platform/<id>` — the only URL we answer. Nothing ships that
    /// posts it yet (the home-screen widget on the x-screentime-gate branch
    /// does); it is kept as the generic "open this platform" entry point.
    ///
    ...
    /// the app to FilteredWebViewContainer, and a deep link and a shield's
    ...
    private func handle(url: URL) {
        ...
        if Platforms.byId(id) != nil {
            ...
            // A route for a platform this build does not carry. There is no
            // feed of ours to open, so the honest answer is the real app,
            // which is where the gate's shield is waiting anyway.
            openRealApp(for: id)
        }
    }
