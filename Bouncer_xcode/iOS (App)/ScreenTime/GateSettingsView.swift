    ...
    var body: some View {
        List {
            ...
            GateSetupSections(gate: gate,
                              gatedHeader: "\(Gate.featureName) platforms",
        ...
        .navigationTitle(Gate.featureName)
...
#if DEBUG
            diagnostics = await gate.diagnostics()
#endif
        }
        .onReceive(NotificationCenter.default.publisher(
            ...
            gate.reconcile()
#if DEBUG
            Task { diagnostics = await gate.diagnostics() }
#endif
        }
    }

#if DEBUG
    /// DEBUG builds only. The `logState` console line, on the screen — the
    /// moment it is needed is a shield button that did nothing, and the phone
    /// in hand usually has no cable. `handoff=` is the load-bearing field:
    /// NEVER means the button never ran, REFUSED means the notification daemon
    /// rejected the post (almost always: permission not granted), accepted
    /// means iOS took it and something downstream chose not to show it.
    private var diagnosticsSection: some View {
        Section("Diagnostics (debug builds only)") {
    ...
    private var armSection: some View {
        if gate.authorization == .approved, gate.hasSelection {
            Section {
                ...
                Toggle(Gate.featureName, isOn: Binding(
                    get: { gate.isArmed },
