...
final class GateController: ObservableObject {
    ...
    // places the user chose to be: the Intentional Scroll slide in onboarding,
    // and Settings → Intentional Scroll. Opening a feed asks for nothing.
    ...
    func logState(_ context: String) {
        print("\(Gate.report(authorization: authorizationLabel)) \(pluginReport()) at=\(context)")
    ...
    /// The same facts `logState` prints, as text a screen can show. Exists for
    /// the DEBUG diagnostics section in GateSettingsView — reading the console
    /// requires a cable, and the moment this is needed is usually a phone in a
    /// hand: shield tapped, nothing arrived, why.
    func diagnostics() async -> String {
        "\(Gate.report(authorization: authorizationLabel)) \(pluginReport())\n"
    ...
    private func logNotificationState(_ context: String) {
        Task {
            print("\(await notificationReport()) at=\(context)")
        }
    }

    private func notificationReport() async -> String {
