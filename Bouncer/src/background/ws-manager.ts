...
function validateWSMessage(raw: unknown): (WSAckMessage & WSResultMessage) | null {
  ...
  // Routing fields must be strings if present
  if (msg.requestId !== undefined && typeof msg.requestId !== 'string') return null;
  if (msg.jobId !== undefined && typeof msg.jobId !== 'string') return null;
  if (msg.error !== undefined && typeof msg.error !== 'string') return null;

  // Must have at least one recognizable field
  if (!('requestId' in msg) && !('jobId' in msg) && !('error' in msg)) return null;

  // Typed fields must have correct types if present
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onmessage = (event: MessageEvent) => {
        try {
          ...
          console.error('[WS Manager] Failed to parse message:', (err as Error).message);
        }
      };
